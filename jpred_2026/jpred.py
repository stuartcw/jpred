...
detects the year from the tables/ directory structure. Also exports aggregated data to CSV files.
...
def main(html_filename, columns_file):
    ...
    if conn is not None:
        ...
        # Determine division from filename (j1, j2, or j3)
        division = Path(html_filename).stem
        
        # Get all teams from the league table for this division
        cur = conn.cursor()
        cur.execute(f"SELECT Team FROM {division}_{year} ORDER BY Position")
        all_teams = [row[0] for row in cur.fetchall()]

        data = {}
        csv_data = {team: {} for team in all_teams}  # Pre-populate with all teams
        
        for column in columns:
            results = query_database(conn, column)
            data[custom_title_case(column)] = results
            
            # Collect data for CSV export - organize by team
            category_name = custom_title_case(column)
            for team, count in results:
                csv_data[team][category_name] = count

        # Export to CSV with teams as rows and categories as columns
        aggregated_dir = Path('aggregated_data')
        aggregated_dir.mkdir(exist_ok=True)
        
        csv_filename = aggregated_dir / f'{division}.csv'
        
        # Convert to DataFrame with teams as index and categories as columns
        df = pd.DataFrame.from_dict(csv_data, orient='index')
        df.index.name = 'Team'
        df = df.fillna(0).astype(int)  # Fill missing values with 0
        
        # Reorder to match position in league table
        df = df.reindex(all_teams)
        
        df.to_csv(csv_filename)
        print(f'CSV file {csv_filename} has been created with aggregated data.')
